   ...
   <body >
      ...
      <main>
         ...
         <div class="col-sm-11 col-centered " id="home-left">
            ...
            <div class="col-xs-12">
    <h1>All Categories by Popular</h1>
    <hr>
</div>
<div class="col-xs-12">
...
<div class="col-xs-12">
    <div class="table-responsive">
        ...
        <tbody>
                ...
                include 'index.php'; //including file for ROOT for DB connection class

                $connect = Database::getConnection(); //giving connection

                $result = $connect->query('SELECT * FROM categories ORDER BY count_queries DESC');
                //getting all categories by decrement way to show the higest first

                while ($row = $result->fetch(PDO::FETCH_ASSOC)) {//making table datas
                    echo '<tr>';
                    echo '<td>'.$row['id'].'</td>';
                    echo '<td>'.$row['title'].'</td>';
                    echo '<td>'.$row['count_queries'].'</td>';
                    echo '<td>'.$row['date'].'</td>';
                }
...
<div class="col-xs-12">
    <div class="table-responsive">
        ...
        <tbody>
                ...
                <?php
                //here we do not need to include index.php again because we already did it
                $result = $connect->query('SELECT * FROM products ORDER BY count_queries DESC');
                //getting all categories by decrement way to show the higest first

                while ($row = $result->fetch(PDO::FETCH_ASSOC)) {//making data table for products
                    echo '<tr>';
                    echo '<td>'.$row['id'].'</td>';
                    echo '<td>'.$row['title'].'</td>';
                    echo '<td>'.$row['count_queries'].'</td>';
